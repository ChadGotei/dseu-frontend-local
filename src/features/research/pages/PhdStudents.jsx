...
const PhdStudents = () => {
  ...
  const departments = Object.keys(phdDataAll.departments).map((key, index) => ({
    _id: String(index + 1),
    key,
  ...
  }, [deptFromQuery, isValidDept, setSearchParams, deptKey, defaultDept]);
  ...
  const students = phdDataAll.departments[deptKey] || [];
  ...
  return (
    <div className="flex w-full my-10 text-gray-800 flex-col md:flex-row md:px-10 px-6 gap-5 lg:gap-10 md:gap-3">
      ...
      <div className="flex-1 flex flex-col">
        <h1 className="text-center text-3xl my-6 font-bold md:block hidden">
          Department of{" "}
          ...
          <table className="min-w-full border border-gray-300 rounded-lg bg-white shadow-md">
            <thead className="bg-blue-500 text-white">
              <tr className="text-xs md:text-sm lg:text-base">
                <th className="px-4 py-2 text-left max-w-1 md:max-w-fit">S.No.</th>
