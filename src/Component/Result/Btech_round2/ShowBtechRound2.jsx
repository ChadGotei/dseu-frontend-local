...
import { changeBtechRound2Status } from "../../../utils/resultservices";
...
const ShowBtechRound2 = () => {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status }) => changeBtechRound2Status(id, status),
    onSuccess: (data) => {
  ...
  if (
    result?.message ===
    "You have not alloted any seat please try again in next round"
  ) {
          ...
          {student.status === "pending" &&
            <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 p-4 rounded-lg text-center max-w-xl">
              ...
              <p className="text-justify">
                The seat confirmation window for Btech round 2 admissions is now closed.
